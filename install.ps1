...
function Test-ExecutionPolicy {
    if (-not((Get-ExecutionPolicy).ToString() -eq "Unrestricted")){
		return "Please first update your execution policy with 'Set-ExecutionPolicy Unrestricted'"
	}
}
		...
		foreach ($category in $packagesByCategory.Keys |Sort-Object) {
			...
			foreach ($package in $packages)
			{
				...
				if ($url){
					...
					$posX= 7 + $checkbox.width
					$linkProjectUrl.Location = New-Object System.Drawing.Point ($posX ,$verticalPosition2)
					...
					# We create a temporary, local variable with the correct URL for this specific link.
